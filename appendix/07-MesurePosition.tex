\chapter{Mesure des positions - Différents essais d'étalonnage}
\label{annexe:MesPos}

\section{Introduction}
Cette annexe traite les différentes approches mises en œuvre afin de calibrer correctement les capteurs de position. Les essais sont présentés dans l'ordre chronologique de leur réalisation. Seule la dernière approche a été conservée dans le chapitre principal (section « Mesure des capteurs de position ») ; les autres tentatives, écartées, sont regroupées ici.

\section{Première mesure des capteurs de position}
La première série d'essais correspond à la configuration matérielle issue du précédent travail de bachelor. Les résultats obtenus sont détaillés dans le \autoref{tab:premier_mesure_pos_cond} et le \autoref{tab:premier_mesure_pos_adc} :
...
\section{Mesure après la calibration du capteur 3}
À la suite de ces résultats, une première calibration par la méthode d'apprentissage "Teach-In" a été effectuée sur le capteur de l'inducteur 3. Une série de mesures, de la sortie du capteur jusqu'à l'ADC, a été réalisée afin d'analyser l'évolution de la tension. Les relevés sont présentés dans le \autoref{tab:teachin_3_capteurs}, le \autoref{tab:teachin_3_cond} et le \autoref{tab:teachin_3_adc} :
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        \multicolumn{5}{|c|}{\textbf{Mesure des positions d'entrefer après l'ADC}}                     \\ \hline
        Inducteur          & Théorique {[}mm{]} & Mesurée {[}mm{]} & Erreur & Erreur relative {[}\%{]} \\ \hline
...
\section{Premier étalonnage de chaque capteur}
...
Une nouvelle procédure d'étalonnage a été mise en œuvre. Des mesures ont été effectuées sans application de la correction initiale. Seule la conversion numérique a été conservée et affichée sur l'interface web (Valeur filtré). Les valeurs mesurées sont consignées dans le \autoref{tab:premier_etalonnage_code_adc} :
...
Ces données ont permis d'établir les courbes présentées dans la \autoref{fig:Prem_tracé_ind1}, la \autoref{fig:Prem_tracé_ind2}, la \autoref{fig:Prem_tracé_ind3} et la \autoref{fig:Prem_tracé_ind4} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/PremierEtalonnage/PremEtaloInd1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/PremierEtalonnage/PremEtaloInd2.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/PremierEtalonnage/PremEtaloInd3.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/PremierEtalonnage/PremEtaloInd4.pdf}
...
Une vérification expérimentale a ensuite été menée, dont les résultats figurent dans le \autoref{tab:correction1_adc} :
...
\section{Mesure après la calibration de tous les capteurs}
...
\begin{figure}[H]
    \centering
    \includesvg[width=1\linewidth]{assets/shared/MaquetteCote}
    \caption{Méthode teach-in en ne levant que l'avant}
...
Pour garantir une bonne homogénéité, les quatre inducteurs ont été soulevés puis abaissés selon la procédure détaillée dans la \autoref{sec:CalibrationCaptPosManuel}.

À l'issue de cette calibration, une nouvelle série de mesures (sans correction) a été réalisée. Les données sont regroupées dans le \autoref{tab:recalib_sans_correction_adc} :
...
Ces nouvelles données ont fait l'objet d'une modélisation graphique afin d'en déduire une fonction de correction à implémenter au sein du microcontrôleur. Les courbes obtenues sont visibles sur la \autoref{fig:Sec_tracé_ind1}, la \autoref{fig:Sec_tracé_ind2}, la \autoref{fig:Sec_tracé_ind3} et la \autoref{fig:Sec_tracé_ind4} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/SecondEtalonnage/SecEtaloInd1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/SecondEtalonnage/SecEtaloInd2.pdf}
    \caption{Graphique du second étalonnage de l'inducteur 2}
    \label{fig:Sec_tracé_ind2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/SecondEtalonnage/SecEtaloInd3.pdf}
    \caption{Graphique du second étalonnage de l'inducteur 3}
    \label{fig:Sec_tracé_ind3}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/SecondEtalonnage/SecEtaloInd4.pdf}
    \caption{Graphique du second étalonnage de l'inducteur 4}
    \label{fig:Sec_tracé_ind4}
\end{figure}

Après intégration de ces nouvelles équations dans le code source, une mesure de vérification a été effectuée. Les résultats sont présentés dans le \autoref{tab:verif_etalonnage_teachin_adc} :
...
\section{Étalonnages polynomiaux écartés (ordres 5 et 3)}
Cette section traite des différents polynômes employés. Ceux-ci, ne s'avérant pas satisfaisants car s'éloignant trop du comportement désiré, ont été regroupés ici.

Afin d'implémenter des équations d'ordre supérieur de manière optimale, la méthode de Horner \cite{Horner} a été employée, comme défini dans l'\autoref{eq:Horner} :
...
Une première tentative a été réalisée à l'aide d'un polynôme d'ordre 5. Les courbes correspondantes sont représentées sur la \autoref{fig:Ind1Pol5}, la \autoref{fig:Ind2Pol5}, la \autoref{fig:Ind3Pol5} et la \autoref{fig:Ind4Pol5} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol5/EtaADCPol5-1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol5/EtaADCPol5-2.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol5/EtaADCPol5-3.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol5/EtaADCPol5-4.pdf}
    \caption{Graphique d'étalonnage de l'ADC de l'inducteur 4 (polynome d'ordre 5)}
    \label{fig:Ind4Pol5}
\end{figure}

Ces modélisations n'étant pas concluantes et semblant restreindre la dynamique du système, le degré du polynôme a été réduit à 3. Cette approche a généré les tracés de la \autoref{fig:Ind1Pol3}, la \autoref{fig:Ind2Pol3}, la \autoref{fig:Ind3Pol3} et la \autoref{fig:Ind4Pol3} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol3/EtaADCPol3-1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol3/EtaADCPol3-2.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol3/EtaADCPol3-3.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/appendix/07-MesurePosition/EtaADCPol3/EtaADCPol3-4.pdf}
