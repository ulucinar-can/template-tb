...
La régulation est le gros de l'interruption principale. Pour la réaliser, une machine d'état doit d'abord passer par les étapes suivantes :
...
Pour la démarer, il est néscessaire de presser le bouton S2 ou l'enclencher depuis l'interface utilisateur :
...
Par la suite, deux stratégies de décollages sont mise en place (Take off). Une première concernant les inducteurs 1 et 2 (Les inducteurs devant) et une seconde concernant les inducteurs 3 et 4 (Les inducteurs arrières). Plusieurs phases sont réalisées pour le bon déroulement de la sustentation. La régulation de courant est toujours active et se situe après la machine d'état.\\
...
Ici, on regarde si la distance entre l'inducteur et le rail change. Le capteur de position vient en réalité mesurer la disstance entre l'indcuteur et le rail. Celle-ci au repos est de 3mm. Lors du décollage, le rail et l'inducteur se rapproche diminuant alors la distance entre les deux. La variable \texttt{Position\_c1\_dec} se situe à 99\% des 3mm (2.97 mm). Lorsque le décollage commence et que la position mesurée devient alors plus petite que la distance de référence, il est possible de passer de l'état 2 à l'état 3. Dans le cas contraire, le courant est encore augmenté par une rampe jusqu'à atteindre un courant suffisament grand pour décoller l'avant. Mais une limite existe qui se trouve à 7.82 A. Il est probable qu'au dela de cela, la maquette puisse prendre des dégats.

Pour simplifier le tout, le courant de l'inducteur 1 est appliqué à l'inducteur 2 afin d'avoir la meme valeur de rampe.\\

C'est dès à présent que l'état 3 rentre en œuvre. Pour les étapes suivantes le code complet ne sera pas présentée, car les étapes pour l'inducteur 1 et 2 sont identique. cependant les explications toucheront uniquement à la régulation de position de l'inducteur 1, car les étapes sont identiques pour l'inducteur 2.
Pour permettre à la régulation de position de suivre et s'adapter en douceur, les étapes suivantes sont appliquées :
...
else
{
    ...
    {
        // ramp from 3mm to 2mm par 0.25 mm
...
Lorsque la position se trouve à une valeur supérieure à 2mm, une rampe pour diminuer la position d'un pas de 0.25 mm est appliqué. Lorsque la position devient plus petite que 2mm, la valeur de 2 mm est fixée.\\

Par la suite, le filtre coupe-bande est employé pour enlever les fréquences parasites :
...
C'est dès à présent que la régulation d'espace d'état intervient :
...
\begin{enumerate}
    \item \texttt{ep1} : On calcule l'erreur entre la position de la consigne et celle mesurée.
    \item \texttt{xr1} : On calcule une intégrale à partir de l'erreur de position et l'erreur sur la force.
    \item \texttt{sum\_vp1} : On calcule l'amortissement à partir de la vitesse et de la position mesurée.
    \item \texttt{fc1\_prim} : On calcule la force totale demandée à partir de la position, l'intégrale, l'amortssiement et la force du poids.
    \item On vérifie que les plages ne sont pas dépassées dans le cas contraire des valeurs sont appliquées.
    \item texttt{fce1} : On calcule l'erreur sur la force à partir de la différence entre la force de la consigne et la force de consigne prime, l'antiwindup et la constante de l'antiwindup. Elle est normalement toujours à 0 sauf lorsque les plages sont dépassées qui sera utilisé comme système d'urgence.
\end{enumerate}


Une observation à faire ici est que la plage n'est que vérifier par la suite. Cela permet entre autre de soit avoir une erreur sur la force de 0 soit d'avoir un moyen de contrer l'erreur si jamais les plages sont dépassées.\\

Pour terminer la stratégie de take-off, les calculs suivants sont réalisés sur les courants :
...
Ces calculs ont pour but d'employer la force pour trouver le courant. Pour obtenir le courant, il faut partir de l'équation de la force émise par l'électroaimant \ref{eq:ForceElectro} :
...
\begin{enumerate}
    ...
    \item k : La constante de l'électroaimant [$\frac{N\cdot m²}{I²}$].
    \item d : La distance de l'entrefer [m].
    \item I : Le courant traversant la bobine [A].
\end{enumerate}

De cette équation, il est possible de récupérer le courant induit dans la bobine en l'isolant de l'équation \ref{eq:CourantElectro} :
...
Pour gagner du temps dans le code, la division par k est fait au préalable d'où la multiplication par k et non pas la division.

De ça, le courant pour la régulation de courant de l'état 3 est obtenue.\\

Une étape qui se fait à partir de l'état 3, est un compteur qui s'active pour permettre d'avoir des pôles plus doux en abaissant la valeur des gains statiques. Pour cela, le code commence à compter pour savoir quand est-ce qu'on décide de changer les gains.
...
La stratégie de take-off pour les inducteurs 3 et 4 sont identiques à celle de la stratégie de take-off 1 et 2. C'est pourquoi celle-ci ne sera pas détaillée. Une différence à noter est l'utilisation d'un gain statique sans intégrateur. Ceci a pour but de donner de la souplesse au code. Cela est fait en mettant le gain statique du régulateur de l'inducteur 3 à 0. Par la suite celui-la est remis à 1 pour aider à maintenir le tout :
...
Une première réflexion sur cette partie-là de l'intéeruption est le fait qu'il est néscessaire de refaire la machine d'état. Le code ne semble pas assez robuste.\\

À la fin de la machine d'état, la régulation de courant est réalisée à partir de la fonction \texttt{PI\_current\_regulator} :
...
Une fois le passage par la régulation de courant réalisée, il est néscessaire de calculer le duty-cycle des signaux PWM employé par les pont en H des inducteurs. Pour cela les lignes de codes suivantes sont écrites :
