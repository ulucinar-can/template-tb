...
\subsection{Main}
Le programme pirncipale tourne dans le main qui suit :
...
Au départ, on initialise le microcontrôleur par les fonctions \texttt{Device\_init} et \texttt{Device\_initGPIO} présentes dans la librairie device.h.\\

Ensuite on initialise les interruptions par les fonctions \texttt{Interrupt\_initModule} et \texttt{Interrupt_initVectorTable} contenue dans la librairie interupt.h (Présente dans le fichier device.h).\\

La boucle qui suit est une calibration des PWM. Grâce à cela, les impulsions émises permettent d'être le plus précis possible.\\

La fonction \texttt{SysCtl\_disablePeripheral(...)} vient geler l'horloge des PWM.\\

Ensuite la fonction \texttt{Board\_init()} vient configurer les périphériques (ADC, PWM, UART, ...) par les réglages de SysConfig.\\
