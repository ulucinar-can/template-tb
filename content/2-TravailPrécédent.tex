...
L'interruption principale étant d'une taille importante, celle-ci sera découpée en plusieurs parties. L'interruption est censé durer 40 $\mu s$. 

\subsubsection{ADC}
\label{subsubsec:ADC}

Les ADC ont un rôle crucial dans la régulation. Ceux-ci permettent de récupérer les informations liées à la mesure du courant et de position.\\

Tout d'abord, une lecture des valeurs des ADC est réalisés :
...
Cette lecture est le résultat brut numérique après que le signal soit passé par le conditionneur(voir \ref{subsec:Conditionneur}).\\

Pour le cas de l'ADC de courant, il faut cependant supprimer l'offset avant que la sustentation ne commence. Pour cela le bout de code suivant permet de réaliser cela :

\begin{minted}{c}
// Lecture de l'offset du conditionnement par rapport a la
// valeur theorique 0A au début de l'interruption (1 fois)
// Adaptation de l'offset : 1.5 / 3.3 * 4095 = 1861.36
// (1.5V correspond a un courant de 0A = point de reference)
...
Cette partie du code va permettre d'enlever l'offset de courant qui est de 1.5 A. Cette offset est dû à l'ADC qui a la possibilité de lire des courants négatifs.
Ce procédé permet également pour réaliser une moyenne au bout de 1000 échantillons (Une durée d'environ 40 ms) qui permet d'avoir un résultat plus robuste face au bruit.
