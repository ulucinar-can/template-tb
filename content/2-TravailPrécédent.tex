...
La maquette est constituée de cinq PCB (une carte principale et quatre cartes de puissance) ainsi que de quatre inducteurs, comme illustré sur les figures \ref{fig:MaquetteUp} et \ref{fig:MaquetteDown}.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Image/Chap.2/SchemFonct.png}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Image/Chap.2/SchemFonct.png}
        \caption{La maquette vue de dessous}
        \label{fig:MaquetteDown}
    \end{minipage}
    \label{fig:MaquetteVues}
\end{figure}
...
\subsection{Capteur de position}
Les capteurs de positions sont des capteurs de

\subsection{Capteur de courant}

\section{Schéma électrique de la carte principale}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth,clip,trim=5mm 0 1mm 0]{Image/Chap.2/Scheme/SchemaComplet.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth,clip,trim= 5mm 0 0 5mm]{Image/Chap.2/Scheme/DSP_scheme.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Image/Chap.2/Scheme/Cond_Scheme.jpg}
...
L'alimentation est en réalité l'alimentation des différentes parties de la carte. Celle-ci sont observables sur l'image \ref{fig:AlimSchema} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth,clip, trim=5mm 0 0 5mm]{Image/Chap.2/Scheme/Alimentation_Scheme.png}
...
\begin{itemize}
    \item Un régulateur de tension à 5V pour l'alimentation du DSP, de l'ESP32 et certains AOP. Il prend en entrée 24V.
    \item Un régulateur de 15V qui sert d'alimentation des capteurs de positions. Il prend en entrée 24V.
    \item Un régulateur pour obtenir une tension de 0.5V employé dans le conditionnement des données des capteurs de courants.
    \item Un régulateur de 24V est employé pour alimenter le reste des régulateurs. Celui-ci prend en entrée le 48V dans les deux positions du switch de précharges pour garantir que la carte est entièrement alimenté.
    \item Un régulateur de 3.3V est employé pour alimenter l'ESP32 et certains AOP. Il prend en entrée le 5V.
\end{itemize}

\subsection{Circuit de précharges}
Le circuit de précharge 

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth, clip,trim=5mm 0 0 5mm]{Image/Chap.2/Scheme/Prech_Scheme.png}
    \caption{Circuits de précharges}
\end{figure}
...
Pour terminer la stratégie de décollage, les calculs suivants sont réalisés pour convertir les forces en consignes de courant (Méthode inverse).
...
\subsection{Interruption de la communication UART}
La communication UART s'appuie sur deux interruptions distinctes : l'une dédiée à la réception des messages (RX) et la seconde à l'envoi (TX). Les routines d'interruption sont les suivantes :
...
__interrupt void INT_mySCI0_RX_ISR(void) // .xxx
{
    ...
    while (SCI_getRxFIFOStatus(SCIA_BASE) > 0) {
        ...
        if (rxIndex < RX_BUF_LEN) {
         ...
         if(c == '\x02'){
            // Marque le dbut de trame
            dataIndex = rxIndex; // Sauvegarde la position pour les donnes
         }
         if(c == '\x03'){
            // Fin de trame - traite les donnes
...
__interrupt void INT_mySCI0_TX_ISR(void)
{
    ...
    if (txIndex >= txLength) {
        SCI_disableInterrupt(mySCI0_BASE, SCI_INT_TXFF); // Fin denvoi
    }
...
La communication est actuellement employée pour assurer la réception de la commande d'enclenchement de la maquette ainsi que l'envoi des variables de position vers l'ESP32 :

\begin{minted}{c}
      // Envoi des donnes UART toute les 40*10e-6 * 25000 = 1 s
      UartCounter++;
...
//          SendFloatAsText(1.234f,2.567f,3.891,4.234f); // Trame de test envoye : "\x02float1,float2,float3float4\x03" avec les float sous cette forme x.xxx
