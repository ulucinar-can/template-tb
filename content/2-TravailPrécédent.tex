...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Image/Chap.2/Scheme/ESP32_Scheme.png}
...
La première phase est en réalité un peu spécial, car elle va s'occuper du décollage sur les 4 inducteurs. Elle va permettre de réaliser un décollage en douceur pour atteindre les 3A.\\

Au départ, on va accumuler la valeur du courant mesurée jusqu'à ce que la variable \texttt{dt\_mean} atteignent 200 cycles (Durée d'environ 8 ms) :
...
Juste après cela, une vérification est réalisée sur les moyennes afin de vérifier si le courant mesuré se situe dans la bonne plage de fonctionnement. Dans le cas où les 4 inducteurs ont bien atteint une valeur proche de la consigne, alors on passe à la seconde phase. Dans le cas contraire, les moyennes et le temps sont remises à 0 et il est une nouvelle fois nécessaires de faire les 200 cycles :
...
En parallèle de cela, une rampe est gentiment créée allant de 0 A à 3A avec un pas de 0.04A. Seulement la valeur du courant de la bobine 1 va être augmenté. Elle a un rôle de "variable maîtresse". C'est elle qui, une fois la valeur de 3A dépasser va venir appliquer un saut indicielle de 3A sur le restant des inducteurs :
