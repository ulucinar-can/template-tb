...
Cette fonction sera principalement utilisée dans l'interruption principale pour la régulation de courant des bobines(voir \red{xxxx}).
...
Le filtre de Savitzky \cite{Savitzky} est employé pour lisser les signaux présentant un bruit important tout en préservant la tendance de la variation du signal, ce qui est crucial pour la précision de la régulation.\\
...
\begin{figure}[H]
    ...
    \caption{Principe du filtre Savitzky}
    \label{fig:PrincipeSavitzky}
\end{figure}

Le filtre Savitzky est en quelque mot un filtre FIR à fenêtre glissante. Le filtre vient garder dans le cas actuel les 9 dernières positions de l'inducteur. Par la suite il vient appliquer une moyenne. La moyenne est réalisée en multipliant chaque position aux coefficients Savitzky qui sont les suivants :
...
Il est possible d'augmenter la taille de la fenêtre
