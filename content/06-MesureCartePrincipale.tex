...
Une première série de mesures a été réalisée sans ajout de masse. Ces résultats sont consignés dans le tableau \ref{tab:sans_poids_comp} :
...
Selon le même principe, des essais ont été effectués en ajoutant une masse d'environ 1,25 kg. Les mesures correspondantes sont présentées dans le tableau \ref{tab:poids_1_25_comp} :
...
Enfin, une charge d'environ 2,5~kg a été ajoutée pour clore les essais. Les résultats de ces mesures sont détaillés dans le tableau \ref{tab:poids_2_5_comp} :
...
Cette section traite des mesures réalisées sur les capteurs de position. L'étalonnage précédent fut réaliser de la manière suivante :
...
L'aproche de multiplier un gain similaire pour les inducteur 3 et 4 laissant a désirer, de nombreuses séries d'essais ont été effectuées afin d'améliorer étalonnages. Par conséquent, seule la dernière approche fut garder afin d'alléger le contenu de ce document. Les autres sont disponible dans l'annexe \ref{annexe:MesPos}.

Il fut également décider de réaliser une nouvelle calibration teach-in des capteurs de positions. Cela est dû au fait la calibration fut réaliser en levant uniquement l'arrière ou uniquement l'avant. Cela avait également pour but d'éliminer un angle qui pouvait se créer entre les capteurs avant et arrière comme indiqué sur la figure \ref{fig:MaqCote} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap3/MaquetteCote.png}
    \caption{Calibration en levant uniquement un côté}
    \label{fig:MaqCote}
\end{figure}

La calibration fut réaliser à une meme hauteur en suivant les étapes décrites dans l'annexe \cite{sec:CalibrationCaptPosManuel} concernant la calibration.
