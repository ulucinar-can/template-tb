...
Par la suite, le courant mesuré par une sonde de courant et la mesure de l'adc sont réalisé. Cela à pour but de les comparer et de savoir si un quelconque défaut se présenterai sur les capteurs de courants lors d'une grande différence entre la sonde et celle-ci.

La mesure se fait en soulevant soit uniquement l'avant soit uniquement l'arrière. Cela avait pour but d'avoir un point de fonctionnement stable et non oscillant avec les quatres inducteurs en fonctionnement. Afin de garantir la mesure plusieurs masses furent employer en les plaçant sur les points élevé de la maquette comme indiqué sur la figure \ref{fig:EmplacementPoids} :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets/Chap3/MesureMasse.jpeg}
    \caption{Emplacement des poids}
    \label{fig:EmplacementPoids}
\end{figure}

Des mesures comparatives entre un calcul théorique selon le poid et la mesure (adc et sonde) furent également réaliser. Celle-ci étant moins parlante et étant réaliser avant la découverte du changement de poids, elle seront placer dans l'annexe \ref{annexe:MesCour}.
