...
La théorie de la synthèse MIMO (efforts modaux, retour d'état, observateur, allocation) est développée au \autoref{subsec:RegPIError}. Cette section se limite à sa traduction en code. Les coefficients employés proviennent du script Python de synthèse, exporté directement dans le fichier \texttt{FunctionHeader.h}.
...
La régulation MIMO s'active dès que l'arrière commence à se déplacer (état 5). Le bloc suivant reprend, dans l'ordre, les étapes de la synthèse du \autoref{subsec:RegPIError} :
...
\begin{enumerate}
    ...
    \item Écrêtage de chaque force entre $0$ et $F_{max}$.
