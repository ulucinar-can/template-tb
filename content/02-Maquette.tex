...
\begin{figure}[H]
    ...
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/Chap2/MaquetteDessus.png}
    ...
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/Chap2/MaquetteCote.png}
        \caption{Vue de côté}
...
Les dimensions de la partie lévitante de la maquette, exprimées en millimètres, sont représentées sur l'image \ref{fig:TailleMaquette} :
...
Les capteurs de courant sont des capteurs LEM \cite{CaptCour}, à effet Hall, disposés sur les cartes de puissance. Leur plage de mesure s'étend de $-30$~A à 30~A (voir tracé \ref{fig:GrapheCaptCour}) et leur bande passante atteint 50~kHz. Ils permettent de mesurer le courant traversant les bobines.
...
Les capteurs de position sont des capteurs Baumer \cite{CaptPosition}, de type optique par triangulation laser. Leur distance de base se situe entre 30~mm et 70~mm (voir tracé \ref{fig:GrapheCaptPos}). Grâce à une fonction d'apprentissage, cette plage peut être réduite afin de se focaliser sur la distance de l'entrefer. La procédure de calibration du capteur est détaillée à la \autoref{sec:CalibrationCaptPosManuel}.
...
Un offset est appliqué au courant afin d'exploiter l'intégralité de la plage de mesure. Sans cette correction, la valeur calculée resterait toujours positive ; la soustraction de l'offset permet de retrouver la partie négative de la caractéristique illustrée à la figure \ref{fig:GrapheCaptCour}.
...
La conversion de la position est simplifiée par l'application d'un étalonnage par apprentissage (\textit{teach-in}) sur le capteur (section \ref{sec:CalibrationCaptPosManuel}). Cette méthode permet de redéfinir la plage de mesure du transducteur : la plage nominale initiale, allant de 30~mm pour 0~V à 70~mm pour 10~V, est ajustée pour correspondre exactement à l'intervalle utile de 0~mm à 3~mm.
