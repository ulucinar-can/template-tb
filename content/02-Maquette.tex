\chapter{Situation de départ - Partie matérielle}

\section{Introduction}
Ce chapitre se concentre sur l'analyse des développements matériels réalisés lors des Travaux de Bachelor précédents, plus particulièrement la conception des cartes de puissance par Mattia Laucella \cite{Laucella} et la refonte de la carte principale par Thomas Freyche \cite{Freyche}. Bien que la révision de la carte principale opérée par ce dernier ait permis de corriger de nombreuses anomalies issues du routage initial de Christophe Isoz \cite{Isoz}, il n'est pas exclu que certains défauts subsistent.

\section{Présentation de la maquette}
La maquette est constituée de cinq PCB, soit une carte principale et quatre cartes de puissance, ainsi que de quatre inducteurs, comme illustré sur la \autoref{fig:MaquetteUp} et la \autoref{fig:MaquetteSide}.

\begin{figure}[H]
    \centering
    % Première vue (Dessus) - ajout de [c]
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/content/02-Maquette/MaquetteDessus.png}
        \caption{Vue de dessus}
        \label{fig:MaquetteUp}
    \end{subfigure}\hfill
    % Deuxième vue (Côté) - ajout de [c]
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{assets/content/02-Maquette/MaquetteCote.png}
        \caption{Vue de côté}
        \label{fig:MaquetteSide}
    \end{subfigure}

    \caption{Vues de la maquette}
    \label{fig:MaquetteGlobale}
\end{figure}

Comme présenté sur la \autoref{fig:MaquetteUp} et la \autoref{fig:MaquetteSide}, la maquette comprend :
\begin{itemize}
...
Une symétrie est adoptée afin d'assurer une répartition de masse équilibrée entre les inducteurs.

Les dimensions de la partie lévitante de la maquette, exprimées en millimètres, sont représentées sur la \autoref{fig:TailleMaquette} :

\begin{figure}[H]
    \centering
    \includesvg[width=1\linewidth]{assets/content/02-Maquette/MaquetteTaille}
...
\begin{enumerate}
    ...
    \item Le rail.
    \item La plaque en bois stratifié blanc servant de référence aux capteurs de position.
...
La partie supérieure et la partie inférieure assemblées constituent l'ensemble lévitant.

Les inducteurs présentent une résistance de 0.8~$\Omega$ et comportent chacun 125 spires. La surface des entrefers est de 1000~mm$^2$ (50~mm $\times$ 20~mm).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{assets/content/02-Maquette/inducteur.jpg}
    \caption{Surface d'un seul inducteur}
    \label{fig:surface_inducteur}
\end{figure}

Les capteurs de courant sont des capteurs LEM \cite{CaptCour}, à effet Hall, disposés sur les cartes de puissance. Leur plage de mesure s'étend de $-30$~A à 30~A (voir la \autoref{fig:GrapheCaptCour}) et leur bande passante atteint 50~kHz. Ils permettent de mesurer le courant traversant les bobines.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets/content/02-Maquette/CaracteristiqueCourant.png}
    \caption{Caractéristique de sortie du capteur de courant}
    \label{fig:GrapheCaptCour}
\end{figure}

Les capteurs de position sont des capteurs Baumer \cite{CaptPosition}, de type optique par triangulation laser. Leur distance de base se situe entre 30~mm et 70~mm (voir la \autoref{fig:GrapheCaptPos}). Grâce à une fonction d'apprentissage, cette plage peut être réduite afin de se focaliser sur la distance de l'entrefer. La procédure de calibration du capteur est détaillée à la \autoref{sec:CalibrationCaptPosManuel}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets/content/02-Maquette/CaracteristiquePosition.png}
    \caption{Caractéristique de sortie du capteur de position}
    \label{fig:GrapheCaptPos}
\end{figure}

De plus amples détails sur l'électronique sont présentés dans la \autoref{sec:SchemEle}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/content/02-Maquette/SchemFonct.png}
    \caption{Schéma fonctionnel}
    \label{fig:SchemFonct}
\end{figure}

Sur le principe, la carte principale assure l'ensemble de la régulation. Elle commande donc les inducteurs pour la lévitation.
...
Actuellement, une alimentation de laboratoire est utilisée. À terme, une alimentation propre au système sera intégrée.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth,page=1]{assets/shared/SchemaElectrique/TB_TFE_Sust_Mag_1_0_0.pdf}
    \caption{Schéma électrique \cite{Freyche}}
    \label{fig:SchemaComplet}
\end{figure}

Cette partie constitue le cœur de la maquette. Elle regroupe :
\begin{itemize}
...
\subsection{DSP}
Le schéma électrique de l'interface du DSP intègre un connecteur destiné à recevoir une carte d'évaluation Texas Instruments \cite{Freyche}. Cette dernière exécute le programme principal régissant la lévitation. Le connecteur assure ainsi la liaison avec le reste de la carte, comme illustré par la \autoref{fig:DspScheme} :

\begin{figure}[H]
    \centering
    \includegraphics[page=8, width=1\linewidth]{assets/shared/SchemaElectrique/TB_TFE_Sust_Mag_1_0_0.pdf}
...
Le connecteur et la carte d'évaluation se présentent comme illustré sur la \autoref{fig:DspEtConnecteur} :
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[trim = 80 100 150 80, clip, width=\linewidth]{assets/content/02-Maquette/CarteEval.jpg}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[trim = 50 50 50 50, clip, width=\linewidth]{assets/content/02-Maquette/Connector.jpg}
        \caption{Connecteur}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth,page=4]{assets/shared/SchemaElectrique/TB_TFE_Sust_Mag_1_0_0.pdf}
...
Cette valeur de crête est ensuite exploitée pour convertir les signaux numériques en grandeurs physiques (courant et position). Les capteurs présentant une caractéristique linéaire, leur comportement modélisé s'exprime par une équation du premier degré :
...
Un offset est appliqué au courant afin d'exploiter l'intégralité de la plage de mesure. Sans cette correction, la valeur calculée resterait toujours positive ; la soustraction de l'offset permet de retrouver la partie négative de la caractéristique illustrée à la \autoref{fig:GrapheCaptCour}.
...
La conversion de la position est simplifiée par l'application d'un étalonnage par apprentissage (\textit{teach-in}) sur le capteur (\autoref{sec:CalibrationCaptPosManuel}). Cette méthode permet de redéfinir la plage de mesure du transducteur : la plage nominale initiale, allant de 30~mm pour 0~V à 70~mm pour 10~V, est ajustée pour correspondre exactement à l'intervalle utile de 0~mm à 3~mm.
...
Cette section détaille les alimentations des différentes parties de la carte. Les différents blocs sont illustrés sur la \autoref{fig:AlimSchema} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth,page=2]{assets/shared/SchemaElectrique/TB_TFE_Sust_Mag_1_0_0.pdf}
...
\begin{itemize}
    ...
    \item Un convertisseur de 24~V \cite{Reg24V} utilisé pour alimenter l'ensemble des autres convertisseurs. Ce dernier est alimenté par le bus de 48~V. Ce bus est également relié au commutateur permettant l'alimentation des cartes de puissance.
...
Le circuit de précharge a pour objectif d'assurer une mise sous tension progressive des cartes de puissance. Le schéma électrique est présenté sur la \autoref{fig:PrechSchem}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth,page=3]{assets/shared/SchemaElectrique/TB_TFE_Sust_Mag_1_0_0.pdf}
...
Les cartes de puissance sont représentées à la \autoref{fig:PowerSchem} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/shared/SchemaElectrique/power2_V2.pdf}
...
\begin{itemize}
    ...
    \item Un capteur de courant destiné à la mesure de l'intensité traversant la bobine.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{assets/content/02-Maquette/ESP32_Board.png}
...
\begin{itemize}
    ...
    \item Convertisseur de tension de 3.3~V : ME6211C33U4AG-N
    ...
    \item Dimensions approximatives du PCB : 37~mm $\times$ 18~mm
\end{itemize}

Le raccordement de la carte est établi selon la \autoref{fig:PinoutESP32} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/content/02-Maquette/ESP32_Scheme.png}
