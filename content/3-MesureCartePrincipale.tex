...
Suite à observation de ces résultats, une première calibration par la méthode Teach-In a été réalisé sur le capteur de l'inducteur 3. Une mesure du capteur jusqu'à l'ADC fut réalisé afin d'observer le comportement de la tension donnant le résultat des tableaux \ref{tab:teachin_3_capteurs}, \ref{tab:teachin_3_cond} et \ref{tab:teachin_3_adc} :
...
Une première tentative d'étalonnage fut réaliser. Pour cela des mesures ont été réalisé sans application de la correction. Seul la conversion fut réaliser et afficher sur l'interface web. La mesure donna lieux aux tableaux \ref{tab:premier_etalonnage_code_adc} :
...
Une vérification de la mesure fut réaliser comme suit dans le tableaux \ref{tab:correction1_adc} :
...
Bien que les erreurs soit faible, le fonctionnement avec l'implémentation de l'équation n'améliorais pas la situation de la lévitation.

\subsection{Mesure après la calibration de tous les capteurs}
Suite à l'étalonnage précédent, une calibration des 4 capteurs fut réalisé. Le but derrière cette démarche fut d'avoir une origine similaire pour chaque inducteur.  :
...
Avec les bloc 7 la plaque en bois, le bloc 8 la carte principale et les blocs 9 les cartes de puissances.
...
Suite a cette calibration, une nouvelle mesure sans les corrections a été mesuré présent dans le tableau \ref{tab:recalib_sans_correction_adc} :
...
Cette mesure donna de nouveau lieux à une tentavie de tracés de courbe afin d'obtenir une correction à appliquer dans le code. Les tracés sont observable dans les figures \ref{fig:Sec_tracé_ind1}, \ref{fig:Sec_tracé_ind2}, \ref{fig:Sec_tracé_ind3} et \ref{fig:Sec_tracé_ind4} :
...
Une fois ces équations insérer dans le code, une mesure de vérification est réalisé repésenté par le tableau \ref{tab:verif_etalonnage_teachin_adc} :
...
Cependant comme précédement, le résultat en marche de la lévitation ne donnait pas suite à une amélioration.
...
\begin{itemize}
    \item Position max : 3 mm
    \item Position min : 0 mm
    \item Valeur max de l'adc : 3722.7
\end{itemize}

Afin d'avoir une correction plus exacte et propre à chaque inducteur, l'équation viendra donc s'appliquer immédiatement sur la valeur de l'adc qui sera convertit en une valeur en mm.\\

Lors de plusieurs mesure, il a été observé que le comportement des capteurs de positions possèdait plus un comportement non linéaire que linéaire. C'est pourquoi au lieux d'essayer d'opter par une courbe de tendance linéaire, une approche d'équation d'un polynôme supérieur fut envisaggé. Ceci donne donc les tableaux \ref{tab:adc_brut_inducteur1}, \ref{tab:adc_brut_inducteur2}, \ref{tab:adc_brut_inducteur3} et \ref{tab:adc_brut_inducteur4} :
...
Par la suite une moyenne de chacune des valeurs a été réalisé pour obtenir le tableau \ref{tab:adc_brut_moyenne}:
...
Afin d'implémenter des équations d'ordre supérieur et sans perdre trop de temps pour le réaliser, la méthode d'Horner \cite{Horner} a été employé démontré dans l'quation \ref{eq:Horner} :
...
Une première tentative fut réaliser avec un polynôme d'ordre 5 représente par les graphiques \ref{fig:Ind1Pol5}, \ref{fig:Ind2Pol5}, \ref{fig:Ind3Pol5} et \ref{fig:Ind4Pol5} :
...
Le résultat n'étant pas concluant et ne semble pas laisser a la maquette d'avoir de la liberté, cet ordre fut baissé à un ordre 3 donnant donc les tracés des graphiques \ref{fig:Ind1Pol3}, \ref{fig:Ind2Pol3}, \ref{fig:Ind3Pol3} et \ref{fig:Ind4Pol3} :
...
Le résultat n'étant toujours pas satisfaisant donna alors un dernier essaie d'une simple équation linéaire donnat les tracés des graphiques \ref{fig:Ind1Lin}, \ref{fig:Ind2Lin}, \ref{fig:Ind3Lin} et \ref{fig:Ind4Lin} :
...
\subsection{Analyse des résultats}
La réalisation du capteurs de positions ne fut pas concluant. Les corrections appliqué donnait régulièrement de faible erreur relative. Théoriquement le procédé semble correcte mais ne donne jamais un résultat satisfaisant.

La dernière correction classique réalisé ne donna également jamais un bon résultat.

Un phénomène récurant lors de la correction fut que les inducteurs venait simplement se coller et se satisfaisait de cette état. Pointant alors plus vers un problème lié a la régulation qu'à la correction des capteurs de positons.

Cependant point intéressant à noter fut qu'après la calibration de tous les capteurs à un meme niveau semble avoir améliorer la stabilité de maintien de la maquette.

\section{Conclusion des mesures}
Les premières mesures réalisé sur l'ensemble des régualteurs donne d'excellent résulats démontrant leur bon fonctionnemnt.\\

Les mesures réalisé sur le fonctionnement des capteurs de courants ainsi que les circuits de conditionnement donne des résultat éloignée de la valeurs théorique mesuré. Les calculs furent réaliser avec la méthode inverser, comme implémenter dans le code, mais les résultats ne semble pas exactement similaire. Cependant la valeur réelle traversant l'inducteur et celle mesuré par le capteurs sont très proche. Il est donc convenable d'assumer que le capteurs de courant fonctionne correctement.\\

La mesure de position donnère régulièrement des mesures prometteuse cependant aucune correction n'aida réellement à la régulation. La seule correction semblant aider à la lévitation de la maquette fut le calibrage par la méthode Teach-In du capteurs en plaçant les inducteurs à un meme niveau.\\

L'électronique est donc confirmé comme fonctionnant correctement.