...
\begin{table}[H]
    ...
    \begin{tabular}{cccc}
        \toprule
        \textbf{Mode} & $j$ & $q_j$        & $I_j$     \\
        \midrule
        Pompage       & $Z$ & $\bar\delta$ & $m_{tot}$ \\
        Tangage       & $T$ & $\theta$     & $J_Y$     \\
        Roulis        & $R$ & $\varphi$    & $J_X$     \\
        \bottomrule
    \end{tabular}
    \caption{Correspondance entre les trois modes, leur coordonnée et leur
        inertie.}
    \label{tab:modes}
\end{table}
...
L'effort $u_j$ déterminé précédement, est le cas parfait. La maquette demande cette effort et la fournit directement. Cependant tous les retards ne sont pas pris en compte. Comme précédement, les différents élement (Filtre coupe-bande, approximation de la boucle fermée de courant, capteurs et filre RC) seront pris en compte dans la régulation. Le filtre Savitzky fut enelver pour plustot employer un observateur sur la vitesse. Cela a pour but de supprimer un élement retardant la boucle de position et de ne pas faire exploser le bruit de la mesure.

Pour inclure ces élement,
