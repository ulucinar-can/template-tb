...
La régulation de courant est synthétisée au moyen d'un correcteur Proportionnel-Intégral (PI). Le schéma bloc de la figure \ref{fig:SchemaBlocRegCur} représente cette architecture :
...
Par conséquent, la composante liée à la position ($\delta$) est négligée afin de rapprocher le comportement du système de celui d'un moteur électrique et de simplifier la régulation de l'intensité. Le système d'équations se réduit à la forme suivante :
...
Pour insérer l'équation de l'accélération dans celle de la variation de courant, la dynamique du courant est dérivée une fois supplémentaire par rapport au temps :
...
\begin{equation}
    \tau_{mi} = \frac{1}{f_{mi}} = \frac{1}{25\cdot 10^3} = 40 \text{ $\mu$s}
\end{equation}
...
\begin{equation}
    \tau_{ii} = N \cdot\sum\tau_{pct} = N \cdot \left(\tau_{cm} + \tau_{mi}\right) = N \cdot \left(40 + 40\right)\cdot 10^{-6} = N \cdot 80\cdot 10^{-6}
\end{equation}
...
\begin{equation}
    \tau_{ii} = 10 \cdot 80 \cdot 10^{-6} = 800 \text{ $\mu$s}
\end{equation}
...
La valeur de $K_{pi}$ retenue est d'environ 33~dB, ce qui correspond à un gain linéaire de 90,96.

La validation du comportement global est effectuée par l'application unitaire d'un échelon en boucle fermée :
