...
Afin de réaliser la dérivé de l'inductance, il est possible de repartir de l'équation simplifié \ref{eq:IndSimple} pour le calcul de l'inductance. Sa derivée donne alors l'équation \ref{eq:IndDerive} :
...
En injectant cette nouvelle équation \ref{eq:IndDerive} et l'équation \ref{eq:IndSimple} dans l'équation \ref{eq:TensionBobineEtape2}, l'équation \ref{eq:TensionBobineEtape3} est obtenue :
...
Il est alors finalement possible d'isoler la dérivée du courant pour obtenir l'équation \ref{eq:CourDerive} :
...
Etant donné que le travail de bachelor essaie de faire léviter une pièce, il est important de faire intervenir les équations de Newton. Celle-ci est définie par l'équation \ref{eq:Newton1} :

\begin{equation}
    m \cdot \ddot{\delta} =\sum F = Fp - F_{el} = m \cdot g - F_{el}
    \label{eq:Newton1}
\end{equation}
...
La force électromagnétique se définit comme étant la dérivée de l'énergie co-magnétique :
...
Il est maintenant possible d'injecter cette équation \ref{eq:Fele} dans l'équation de Newton \ref{eq:Newton1} donnant alors la nouvelle équation \ref{eq:Newton2} :
...
Pour terminer, il est maintenant possible d'isoler l'accéleration pour obtenir cette équation finale :
