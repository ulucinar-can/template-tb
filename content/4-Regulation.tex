...
La régulation de courant vient être synthétiser par un PI. Le schéma bloc de la figure \ref{fig:SchemaBlocRegCur} représente cette régulation :
...
Afin de réaliser cela, il est néscessaire de réaliser la synthèse du système. Celle-ci est réaliser en faisant la mise en commun des équations linéariser précédement :
...
Dans ce modèle, l'accélération ne dépend pas de la position angulaire du rotor. En appliquant ce même principe au système de sustentation, il est possible de supposer, par comparaison des deux dynamiques, que l'influence de la position peut être omise. L'équation mécanique linéarisée complète s'écrit :
...
Par conséquent, la composante liée à la position ($\delta$) est négligée afin de rapprocher le comportement du système de celui d'un moteur électrique et ainsi simplifier la régulation du courant. Le système d'équations se réduit à la forme suivante :
...
Pour insérer l'équation de l'accélération dans celle de la variation de courant, il suffit de dériver une fois supplémentaire la dynamique du courant par rapport au temps :
...
Le bloc $G_{cm}$, représentant l'organne de commande, est approximer par une filtre passe-bas du premier ordre :

\begin{equation}
    G_{cm} = \frac{K_{cm}}{1+s \cdot\tau_{cm}} = \frac{1}{1+s \cdot\tau_{cm}}
\end{equation}

La maquette possède une modulation PWM numérique. Celle-ci vient alors dicter la constante de temps de l'organne de mesure $\tau_{cm}$ correspondant alors à la période d'échantiollonage :

\begin{equation}
    \tau_{cm} = h = 40 \text{ $\mu s$}
\end{equation}

L'organne de mesure du courant $G_{mi}$ est également approximer par un filtre passe-bas d'ordre 1 :

\begin{equation}
    G_{mi} = \frac{K_{mi}}{1+s \cdot\tau_{mi}} = \frac{1}{1+s \cdot\tau_{mi}}
\end{equation}

Afin de déterminer la constante de temps du capteur, il est néscessaire de fouiller dans la datasheet du fabriquant \cite{CaptCour}. Celle-ci nous donne une bande passante à 50 kHz. La constante de temps vallant alors la bande passante. Afin de respecter le théorème de Shanon \cite{Shanon}, la fréquence fut choisit à 25 kHz correspondant à la fréquence d'échantiollange du DSP réalisant ainsi un filtre d'anti-repliement :

\begin{equation}
    \tau_{mi} = \frac{1}{f_{mi}} = \frac{1}{25\cdot 10} = 6.4 \text{ $\mu s$}
\end{equation}

Il est maintenant possible d'écrire le système à régler :
...
Une fois cette fonction implémenter dans Matlab, le diagramme de bode peut-être tracé. Cependant, une erreur majeur fut trouver dans les synthèses de la régulation de courant précédente. En effet une constante fut oublier d'être multiplier dans le système $G_{ui}$ comme observé dans le code suivant :
...
Ceci à pour cause d'avoir un système à régler complétement différent. Comme observé sur les tracé suivant, on observe que le début du tracé débute en réalité non pas vers 0 dB mais vers les -40 dB :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/BodeGaiFaux.png}
    \caption{Système à régler sans la constante multiplier}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/BodeGaiVrai.png}
    \caption{Système à régler avec la constante multiplier}
\end{figure}

Pour la suite, les différents tracé se concentrerons sur le système correcte, c'est à dire le système ayant la constante multiplier.\\

La fonction de transfert du régulateur est donné par l'équation \ref{eq:PI}
...
Le système en boucle ouverte $G_{BO}$ est alors définit comme suit :

\begin{equation}
    G_{BO} = G_{ci}\cdot G_{ai} = K_{pi}\cdot\frac{1+s\cdot\tau_{ii}}{s\cdot\tau_{ii}}\cdot K_{ui}\cdot\frac{s}{1 + a_1 \cdot s + a_2\cdot s^2} \cdot \frac{1}{1+s \cdot\tau_{cm}} \cdot \frac{1}{1+s \cdot\tau_{mi}}
\end{equation}

Il faut dès à présent trouver les valeurs de la constante de temps du régulateur $\tau_{ii}$ ainsi que son gain $K_{pi}$.\\

Pour commencer, la constante de temps se définit comme étant la somme des petites constantes de temps multiplier par une constante $N$. Les petites constantes de temps correspondents à la somme de $\tau_{cm}$ et $\tau_{mi}$ :

\begin{equation}
    \tau_{ii} = N \cdot\sum\tau_{pct} = N \cdot \left(\tau_{cm} + \tau_{mi}\right) = N \cdot \left(40 + 6.4\right)\cdot 10^{-6} = N \cdot 46.4\cdot 10^{-6}
\end{equation}

Pour définir un N qui convient, un tracé avec plusieur N différent allant de 5 à 15 fut tracé. Une fois-cela réaliser, la bande passante ainsi que l'erreur fut tracé pour trouvé un bon compromis pour ne pas avoir trop d'erreur et avoir un système suffisament rapide. Ceci est observable dans figure de \ref{fig:DiffN} à \ref{fig:Erreur} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/TraceN.png}
    \caption{Tracé de plusieurs du système régler pour plusieurs N}
    \label{fig:DiffN}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Tracé de la Bande-Passante pour différentes valeur de N}
    \label{fig:BP}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Tracé de la valeur de l'erreur pour différentes valeurs de N}
    \label{fig:Erreur}
\end{figure}

La valeur choisit ayant un bon compromis est à N=10. Ceci donne alors la constante de temps suivante :

\begin{equation}
    \tau_{ii} = 10 \cdot 46.4 \cdot 10^{-6} = 464 \text{ $\mu s$}
\end{equation}

Maintenant que la valeur de $N$ est choisie, il est possbile de trouver la valeur du gain $K_{pi}$ pour une marge de phase de 45° :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/Kpi45.png}
    \caption{Tracé de $K_{pi}$ pour une marge de phase de 45°}
\end{figure}

La valeur de $K_{pi}$ retenue est alors d'environ 33 dB. La valeur linéariser est alors de 90.96.

Il est possible de vérifier le comportement global en réalisant un saut linéaire. Pour réaliser cela, la boucle fermée est tout d'abord réaliser et ensuite le saut linéaire est appliqué :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/BF1.png}
    \caption{Tracé de la boucle fermé}
\end{figure}

Pour cette boucle fermée, le résultat du saut indicielle donne :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/Step.png}
    \caption{Saut indicielle de la régulation de courant}
\end{figure}

Le système est alors confirmé.\\

Pour terminer, une approximation de ce système par un filtre d'ordre 1 est réalisé. Cela à pour but de faire une approximation qui sera employé dans la régulation de position. Pour cela, la constante de temps de la boucle fermée est recherché à -45°. Ceci donne alors la figure \ref{fig:BFPI} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/BF.png}
    \caption{Recherche de la constante de temps de la boucle fermée}
    \label{fig:BFPI}
\end{figure}

La constante de temps de la boucle fermée $\tau_{iBF}$ est alors de 78.32 $\mu s$.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/ApproxPI.png}
    \caption{Diagramme de l'approximation de la boucle fermée}
\end{figure}

Il faut cependant faire une comparaison entre les valeurs avec et sans la constante $K_{ui}$.
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Comparaison des paramètres avec et sans la multiplication de $K_{ui}$}
    \label{tab:comparatif_kui}
\end{table}

Ce que l'on peut constater, la seule valeur changeant est celle du gain $K_{pi}$. En effet l'ajout d'une constante ne vient pas influancer la phase puisque celle-ci est de 0°. On peut observer cela dans les figures \ref{fig:BodeKpiFaux} et \ref{fig:BodeKpiFaux} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/BodeKpiFaux.png}
    \caption{Diagramme de la recherche de $K_{pi}$ sans la multiplication}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/Chap4/BodeKpiVrai.png}
    \caption{Diagramme de la recherche de $K_{pi}$ avec la multiplication}
\end{figure}

Mais lorsque la valeur de 90.96 est insérer dans le code, celui-ci vient se coller fortement au rail. Cela s'explique par le fais que les carte de puissances ont été réaliser pour le gain précédent. Si la valeur de 90 est présente laors une petite erreur viendrait donner une immense tension aux bornes de l'inducteur. Mais la tension d'entrer est limiter à 48V et 3.5 A. Il est alors pas possible de dépasser ces limites mais c'est belle est bien ce que le système demande pour un gain aussi grand. D'ou le fait d'un meilleur fonctionnement pour la valeur de 1.88 de $K_{pi}$.

Il faut alors touiller cette valeur dans le code afin de trouver une valeur suffisament grande pour garantir un système rapide mais non plus trop grande afin d'éviter le problème du rail coller.

Travail à faire par la suite :
\begin{itemize}
    \item Chercher une bonne valeur de Kpi
\end{itemize}
