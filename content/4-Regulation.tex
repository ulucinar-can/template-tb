...
De cette equation \ref{eq:Fele}, il est possible d'écrire l'équation de la méthode inverse :
...
Avec la constante $k$ valant :

\begin{equation}
    k = \frac{L_n\cdot\delta_n}{2} = \frac{4.9\cdot 10^{-3}\cdot 2\cdot 10^{-3}}{2}=4.9\cdot 10^{-6} \text{ }N\cdot m^2/A^2
\end{equation}

L'équation \ref{eq:MethInv} sera principalement employé dans le code ainsi que pour calculer différentes valeurs lors de la linéarisation \ref{subsec:lin}.
...
\begin{itemize}
    ...
    \item Une tension nominal est brancher sur les bobines.
\end{itemize}
...
Afin de calculer le courant, il faut repartir de l'équation \ref{eq:Fele}. En utilisant les valeurs nominal, l'équation devient :

\begin{equation}
    F_{el}(\delta_n) = i^2 \cdot \frac{L_n\cdot\delta_n}{2\cdot\delta_n^2} = i^2 \cdot \frac{L_n}{2\cdot\delta_n} = \frac{L}{1}
\end{equation}

En utilisant dès à présent l'équation \ref{eq:CourantElectro} avec les valeurs nominaux, on obtient le courant suivant :

\begin{equation}
    i_n = \sqrt{\frac{F}{k}}\cdot\delta
\end{equation}
