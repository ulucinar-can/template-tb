...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets/content/01-introduction/L0-950.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets/content/01-introduction/Swissmetro_Network_2005.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets/content/01-introduction/Magnetschwebebahn.png}
    \caption{Principe de l'EMS \cite{Swissmetro}}
\end{figure}

\section{Logiciels employés}
Les logiciels employés durant ce Travail de Bachelor sont les suivants :

\begin{itemize}
    \item Code Composer Studio pour la programmation du DSP.
    \item MATLAB pour les simulations et la synthèse des régulateurs.
    \item Visual Studio Code pour la rédaction du rapport en \LaTeX{} ainsi que pour l'écriture des scripts Python.
    \item Microsoft Excel pour la planification de Gantt, le traitement des mesures et la génération des graphiques.
    \item Draw.io pour la conception des schémas et des diagrammes.
    \item Arduino IDE pour le développement du code de l'ESP32.
    \item Tracker pour l'analyse vidéo et la caractérisation des oscillations de la maquette.
\end{itemize}

L'utilisation de Code Composer Studio requiert différentes installations et configurations préalables. Celles-ci sont détaillées dans la \autoref{sec:MiseEnPlace}.
...
\begin{itemize}
    ...
    \item L'ajout d'une action intégrale sur la régulation de l'inducteur 3 engendre une instabilité du système.
    ...
    \item Une erreur a été introduite lors de la phase de synthèse des régulateurs.
    \item La condition de transition de l'état 2 à l'état 3 de la machine d'états, basée sur un seuil de position de 99~\%, n'est pas suffisamment robuste et peut être déclenchée prématurément par un bruit d'acquisition.
    ...
    \item L'inductance nominale calculée est incorrecte.
    \item L'erreur n'est pas calculée correctement pour la boucle de régulation de position.
\end{itemize}

\section{Informations relatives à l'utilisation de la maquette}
\label{sec:usage_maquette}
Un manuel d'utilisation a été élaboré afin d'expliciter les procédures de mise en service et d'encadrer les différentes manipulations réalisables sur le système.

Ce document peut être consulté à l'\autoref{annexe:Manuel}.
