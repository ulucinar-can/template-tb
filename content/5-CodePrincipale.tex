...
Cette section traite du fichier principal (\texttt{hrpwm\_ex1\_duty\_sfo.c}) développé pour réaliser une sustentation. Son programme étant danse, l'entiereté de celui-ci ne sera pas présenter. Seul certaines parties considérés comme importantes seront détaillé.
...
Ces fichiers regroupe l'ensemble des définitions de constantes et des prototypes de fonctions. Les éléments principaux sont :
...
Les fichiers sont une seconde version du travail réaliser par Thomas Freyche. Il a été décidé de séparer les deux fichiers pour obtenir un résultat plus propre et plus aérer. Dans l'emsemble, le fonctionnement reste le meme.
...
\begin{figure}[H]
    ...
    \caption{Principe du filtre de Savitzky \cite{Savitzky}}
...
Les coefficients employée sont actuellement les suivantes :
...
Lors d'analyse précédente, il a été constaté que ces valeurs n'étais pas employé mais d'autre valeur de coefficient furent employé étant les suivantes :
...
Cela provenait très certainement d'essaie précédente. Pour rester en concordence avec les travaux précédents, les coefficient du filtre à 75Hz furent dès alors employer.

\section{Mesure du temps de l'interruption principale}
Une des premières suposition réaliser fut que le code ne respectait pas le temps mis à disposition pour réaliser l'interruption complète. Il a fallut alors vérifier si les temps était toujours à jours ou non, vérifier le temps consommer par chaque partie de l'interuption et vérifier si l'interruption lié à la communication entre le DSP et l'ESP-32 venait à déranger l'interruption principal. La mesure fut dont réaliser pour deux cas. Un premier cas en vérifiant le comportement avec la communication UART et une fois sans. Pour rappel, l'interuption principal dure 40 $\mu$s.\\

Cette meusre fut alors réaliser dans le tableau suivant :
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|l|c|c|c|c|c|c|}
            ...
            \multicolumn{1}{|c|}{}                                 & 4 inducteurs [$\mu$s]                   & Inducteurs 1\&2 [$\mu$s]                & Inducteurs 3\&4 [$\mu$s] & 4 inducteurs [$\mu$s] & Inducteurs 1\&2 [$\mu$s] & Inducteurs 3\&4 [$\mu$s] \\ \hline
    ...
    \label{tab:verif_temps}
\end{table}

Les mesure de temps sont disponible dans l'annexe \ref{an:MesTemps}.

La mesure vient alors prouver le bon fonctionnement et du respect du temps de l'interruption principale.
