...
\section{Réalisation d'une nouvelle machine d'état}
La machine d'état précédente étant compliqué à lire et allant dans tous les sens, la décision de la refaire au propre à été réaliser. Le comportement à suivre est le suivant :

\begin{itemize}

    \item Etat 0 : On attends la pression du boutton

    \item Etat 1 : On monte le courant des inducteurs 1 et 2 à 3 A (pour ne pas partir de zéro).

    \item Etat 2 : On augmente le courant des inducteurs 1 et 2 avec une rampe "infinie" (régulation de courant seule).

    \item Etat 3 : Lorsque l'inducteur 1 et 2 commencent à bouger, on les passe en régulation de position (lévitation de l'avant du véhicule).

    \item Etat 4 : On monte le courant des inducteurs 3 et 4 à 3 A (pour ne pas partir de zéro).

    \item Etat 5 : On augmente le courant des inducteurs 3 et 4 avec une rampe (régulation de courant seule).

    \item Etat 6 : Lorsque l'inducteur 3 et 4 commencent à bouger, on les passe en régulation de position (lévitation de l'arrière du véhicule, l'avant lévite déjà).

    \item Etat 7 : On change les gains statiques pour une régulation plus douce et on passe à une régulation de maintien

    \item Etat 8 : On arrête la sustentation et rénintialise les variables

    \item Etat 9 : On affiche un message d'erreur si la maquette rencontre un problème
\end{itemize}

En suivant ces informations, le schéma présent dans la figure \ref{fig:MachineDEtat} fut réaliser :
...
La machine d'état est également disponible dans l'annexe \ref{an:MachineDetat} pour obtenir plus de détail.\\
