...
La mesure se raélise en se basant sur la méthode détailler dans \autoref{annexe:MethodeMesurePosition}.

La mesure étant réaliser sur une période bien plus longue, la mesure complète sera disponible dans l'\autoref{annexe:MesureComplete}. Les résultat présent ici se concentreront sur 10 sec comme présente sur les figures \ref{fig:AnalyseInd1} à \ref{fig:AnalyseInd4} :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{assets/content/09-MesureInductance/AnalyseInd1.png}
    \caption{Mesure sur 10 sec de la régulation de position sur l'inducteur 1}
...
\begin{figure}[H]
    ...
    \caption{Mesure sur 10 sec de la régulation de position sur l'inducteur 2}
...
\begin{figure}[H]
    ...
    \caption{Mesure sur 10 sec de la régulation de position sur l'inducteur 3}
...
\begin{figure}[H]
    ...
    \caption{Mesure sur 10 sec de la régulation de position sur l'inducteur 4}
    \label{fig:AnalyseInd4}
\end{figure}

Une fois cela réaliser, il est possible de réaliser une analyse sur les forces comme dans la \autoref{fig:AnalyseForce} :
...
Il est possible de réaliser les écarts présent dans le \autoref{tab:forces_inducteurs_v2}
...
Comme constaté une, la force mesuré varie encore une fois de celle suppossée théorique. En fonctionnement complet (4 inducteurs), il est possible de constater que le système oscille enormément. cel revient sur l'une des explications précédentes. Les inducteurs agissent l'un sur l'autre empechant chacun d'obtenir une stabilité.

La régulation de position sera dès lors modifier afin de ne pas réaliser une régulation par inducteur (sysrème en SISO) mais réaliser une régulation de position dans l'espace (système en MIMO).

La composante intégrale de l'inducteur 3 déscends en pente, cela est dû au fait que sont intrégale a été retirer.

\section{Conclusion}
La mesure de l'inductance a démontré que les valeurs d'inductance sont effectivement supérieures à celle calculée. L'hypothèse de la \autoref{sec:Hypothese} selon laquelle l'inductance nominale calculée est incorrecte est donc \textbf{confirmée}. Il sera dès lors possible d'employer ces valeurs mesurées afin d'améliorer l'état actuel de la maquette, notamment pour la synthèse de la régulation de courant et la méthode inverse.

La seconde mesure indique une nouvelle fois une instabilité de la régulation de position. Une approche différente sera traiter pour réaliser la régulation puisque celle-ci montre ses limites.